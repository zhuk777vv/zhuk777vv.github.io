...
if(macroBefore){macroBefore.src='assets/glass-before-detail.webp';macroBefore.width=1200;macroBefore.height=900;}
if(macroAfter){macroAfter.src='assets/glass-after-detail.webp';macroAfter.width=1200;macroAfter.height=900;}
$$('.before-after img').forEach(img=>{img.width=2400;img.height=1500;});

// Every Telegram CTA is a plain native link to one verified contact.
$$('a[href*="t.me/"]').forEach(link=>{
  link.href=TELEGRAM;
  link.removeAttribute('target');
  link.removeAttribute('rel');
  link.dataset.telegram='zhukov_boss';
});
...
function animateMoney(el,to,duration=420){
  if(!el)return;
  ...
  const tick=now=>{
    ...
    el.textContent=fullRub(value);el.dataset.current=String(value);
    if(t<1)el._raf=requestAnimationFrame(tick);else{el.textContent=fullRub(to);el.dataset.current=String(to);}
  };
...
function setSplit(pos,instant=false){
  ...
  if(instant){after?.style.setProperty('transition','none');splitLine?.style.setProperty('transition','none');requestAnimationFrame(()=>{after?.style.removeProperty('transition');splitLine?.style.removeProperty('transition');});}
...
if(demo){
  ...
  if('IntersectionObserver'in window){const io=new IntersectionObserver(entries=>{if(!entries[0].isIntersecting)return;io.disconnect();if(matchMedia('(prefers-reduced-motion: reduce)').matches)return;setTimeout(()=>{if(!userTouched)setSplit(88);},260);setTimeout(()=>{if(!userTouched)setSplit(12);},1450);setTimeout(()=>{if(!userTouched)setSplit(52);},3150);},{threshold:.42});io.observe(demo);}
}

// Fixed economics panel: the total and the current cost remain visible throughout section 02.
...
function updateLoss(){
  ...
  const active=rect.top<innerHeight-86&&rect.bottom>86;
  hud.classList.toggle('active',active);
  const trigger=Math.min(innerHeight*.61,540);
  let idx=-1;
  ...
  const label=current?.dataset.label||'Начало расчёта';
  ...
  if(lossDays)lossDays.textContent=pluralDays(days);if(lossLabel)lossLabel.textContent=label;if(lossBar)lossBar.style.width=(idx<0?0:(idx+1)/lossSteps.length*100)+'%';
  hudDays.textContent=pluralDays(days);hudLabel.textContent=label;hudDelta.textContent='+'+fullRub(delta);hudBar.style.width=(idx<0?0:(idx+1)/lossSteps.length*100)+'%';hudDots.forEach((dot,i)=>dot.classList.toggle('on',i<=idx));
}
...
function calculate(){
  ...
  let replLow,replHigh;if(known>0){replLow=Math.round(known*.97);replHigh=Math.round(known*1.03);}else{let replacement=(14800*area+23500*pieces+42000)*object*Math.max(1,.94*access);if(urgency>1.25)replacement*=1.08;replLow=Math.round(replacement*.9);replHigh=Math.round(replacement*1.14);}
  const saveLow=replLow-restHigh,saveHigh=replHigh-restLow;replacementRange.textContent=compactRange(replLow,replHigh);restorationRange.textContent=compactRange(restLow,restHigh);
  ...
  calcVerdict.textContent=verdict;savingMoney.textContent=moneyText;savingPercent.textContent=percentText;return{d,verdict};
}
...
$('#sendCalculation')?.addEventListener('click',()=>{
  const r=calculate(),d=r.d,object=form.object.options[form.object.selectedIndex].text,damage=form.damage.options[form.damage.selectedIndex].text,access=form.access.options[form.access.selectedIndex].text,urgency=form.urgency.options[form.urgency.selectedIndex].text;
  const text=`Здравствуйте. Хочу понять, можно ли восстановить стекло без замены.\n\nОбъект: ${object}\nПлощадь: ${d.get('area')} м²\nКоличество стёкол: ${d.get('pieces')}\nПовреждение: ${damage}\nДоступ: ${access}\nКогда нужно начать: ${urgency}\n\nОриентир по замене: ${replacementRange.textContent}\nОриентир по восстановлению: ${restorationRange.textContent}\nПо расчёту: ${r.verdict}\nРазница: ${savingMoney.textContent}${sourceLine()}\n\nСейчас пришлю фотографии.`;
  showToast('Открываем Telegram @zhukov_boss');
  location.assign(TELEGRAM+'?text='+encodeURIComponent(text));
});
